/* 課題10-2  砂糖水をシャッフルするプログラム */

public class SatouMizuShuffle {
    public static void main(String[] args){

        Mizu[] card;
        card = new Mizu[14];
        int i;
        int tim;    //10回繰り返す処理をカウントする変数
        ...
        */
        ...
        for (tim = 0; tim < 10; tim++) {    //10回繰り返す処理を行う
            for (i = 0; i <= 13; i++) {     //cardを並び替える処理を行う
                temp[i] = card[(i+5)%14];   //iへ5を足して14で割った余りを新しいcardとする(0から13の範囲をループできる)
            }

            for (i = 0; i < 14; i++) {      //tempをcardへコピーする
                card[i] = temp[i];
            }
        }

        /* card[0], card[1], ..., card[13]を表示する。*/
        for (i = 0; i < 14; i++) {
